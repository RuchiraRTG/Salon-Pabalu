...
export default class FeedbackHome extends Component {
  ...
  retrievePosts() {
    axios.get("http://localhost:5000/feedback/getActiveFeedbacks").then((res) => {
      if (res.data.success) {
        this.setState({
          posts: res.data.feedbacks, // Store feedback data in the posts state
        });
      }
    });
  }
  ...
  onDelete = (id) => {
    ...
    if (confirmDelete) {
      axios
        .delete(`http://localhost:5000/post/delete/`)
        .then((res) => {
            ...
            this.state.posts.map((post, index) => (
              <Feed_Card key={index} className="feed_post-card">
                ...
                <div className="feed_post_container">
                  <div className="feed_post_details">

                    <div>Course Name: {post.courseName}</div>
                    <div>Review: {post.review}</div>

                    {!expanded[post._id] && (
                      <button
                        onClick={() => this.toggleExpand(post._id)}
                        className="feed_see-more"
                      >
                        See More
                      </button>
                    )}
                    ...
                    {expanded[post._id] && (
                      <div className="feed_post_expanded">
